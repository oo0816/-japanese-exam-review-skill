...
**核心原则**：中文母语写作，不可从 jp_first 的 translation 字段机械转换。cn 内容独立撰写，以中文读者的阅读习惯组织逻辑。

每个 section 元素同上，区别在于 concept_card 结构：

1. **章节标题** — 12pt #283c5a 加粗，带 ★ 重要度标记
2. **【Keywords】标签行** — 7pt #283246 加粗
3. **中文正文** — 8.5pt #323232（SimSun 宋体）。以中文为母语写作，内容组织符合中文阅读习惯（先总后分、先定义后展开）。
4. 无独立翻译段落（中文即是主语言）

**段落切分规则（强制执行）**：

- **一个逻辑点 = 一个段落**。段落之间用 `\n\n` 分隔，PDF 渲染时产生清晰的行间距
- **绝对禁止单 `\n`**：reportlab 将单 `\n` 视为空格，不会产生视觉换行。所有换行必须使用 `\n\n`
- 以下情况必须独立分段：
  - 每个 ①②③ 编号条目
  - 每个【小标题】引导的知识点
  - 定义 → 展开 → 举例，三者各自独立成段
...
- **格式模板**：`【小标题】\n\n正文内容`、`① 条目名\n\n说明文字`、`• 要点\n\n• 要点`
- 禁止把多个独立知识点塞进同一个段落
- 编号条目（①②③⑤）每个都必须独立成段，不得用"①...②...③..."形式串联在一个段落内
- 对比性描述（"电路交换...分组交换..."）各自独立成段
- 段落长度建议：3-6 句为佳，超过 8 句需拆分

**术语标注规则（强制执行）**：

- 格式：`<b>中文术语</b>（English / 日本語）`，英文在前、日文在后，缺一不可
- **仅在首次出现时标注**，同一 concept_card 内再次出现仅用中文
- **禁止重复标注**：若原文已含括号说明如 `（Protocol）`，标注时不可叠加为 `（English / 日本語）（Protocol）`
- 标注插入位置：紧跟术语首次出现的那个词，不额外添加括号
- 术语表第 3 列标题为「中国語（English）」，第 4 列説明必须同时包含日文原文和中文翻译，格式：`日文説明（中文翻译）`

**翻译完整性（强制执行）**：

- 所有影响阅读理解的内容必须翻译为中文：overview、concept_card content、term_table 説明列、highlight_box content、exam_tip、key_point
- 术语表説明列：日文原文 + 中文翻译双语呈现
- highlight_box：label 和 content 均需中文
- 可以保留日文原文的元素：术语表第 1 列（日本語）、第 2 列（読み方）、第 3 列术语名
...
6. **表格使用**（按需判断）：
   ...
   - 表格内容必须**日中双语**，所有影响阅读理解的单元格（説明列、对比内容、标题行除外）必须有中文翻译紧跟日文原文
   - 不要强行把所有内容都塞进表格
